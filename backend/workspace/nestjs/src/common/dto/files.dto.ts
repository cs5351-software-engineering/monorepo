...
export class FileDtoSingle implements UserFile{
    ...
    @ApiProperty({ description: 'Result type: it can be "scan_result", "source_code"', required: true })
    @IsString()
    result_type: string;
    ...
    @ApiProperty({ description: 'Replace the original file name', required: true })
    @IsString()
    fileName: string
  }
...
export class FileDtoMultiple implements UserFile{
    ...
    @ApiProperty({ description: '"0" or "1": when user upload multiple files, determine files store at MinIO directly, or "zip them into 1 files":', required: true })
    zip?: string;
  
    //use as the zip file name
    @ApiProperty({ description: 'use to determine the zipped file name', required: true })
