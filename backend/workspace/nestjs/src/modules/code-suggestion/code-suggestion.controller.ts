...
export class CodeSuggestionController {
  ...
  async infillPython(@Body('filePath') filePath: string) {
    const fileContent = await this.codeSuggestionService.startCodeInfill(
      this.ollamaService,
      filePath,
    );
    return fileContent;
  ...
  async getCodeReview(@Body('filePath') filePath: string) {
    const fileContent = await this.codeSuggestionService.startCodeReview(
      this.ollamaService,
      filePath,
    );
    return fileContent;
  ...
  async getUnitTest(@Body('filePath') filePath: string) {
    const fileContent = await this.codeSuggestionService.startGetTestCase(
      this.ollamaService,
      filePath,
    );
    return fileContent;
