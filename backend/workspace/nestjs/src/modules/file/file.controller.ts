import { Controller, Post, Get, Body, Res, UploadedFile, UploadedFiles, UseInterceptors, ValidationPipe, HttpException, HttpStatus } from '@nestjs/common';
...
import { GetFilePath, UpdateFilePath } from '../../common/functions/files-utils';
...
export class FileController {
  ...
  async uploadSingleFile(@UploadedFile() file: Express.MulterFile, @Body(new ValidationPipe) body: FileDtoSingle,) {
    try{
  ...
  @Get('download/single')
  async downloadSingleFile(@Body(new ValidationPipe) body: FileDtoSingle, @Res() res: any) {
    ...
    try{
      ...
      const stream = await this.minioService.downloadFile(user_name, location, fileName);
      //(filePath);
  ...
  async uploadMultipleFile(@UploadedFiles() files: Array<Express.MulterFile>, @Body(new ValidationPipe) body: FileDtoMultiple) {
    try{
      ...
      if (zip == undefined || zip != '1') {
        ...
        await this.minioService.uploadFile(user_name, location, zipFile, zippedFileName);
  ...
  async downloadMinioDirectory(@Body(new ValidationPipe) body: FileDtoMultiple, @Res() res: Response) {
    try{
      const {user_name, result_type, project_id, version, zip, zippedFileName} = body;
      const zipFilePath = join(tmpdir(), `${Date.now()}-files.zip`);
      //get MinIO file location 
      const location =UpdateFilePath(GetFilePath(result_type, project_id, version));
  ...
  async zipMinioDirectory(bucketName: string, prefix: string, outPath: string): Promise<void> {
    const archive = archiver('zip', { zlib: { level: 9 } });
    ...
    if (objects.length > 0) {
      return new Promise((resolve, reject) => {
        ...
        objects.forEach((obj, index) => {
          this.minioService.getObject(bucketName, obj.name)
            .then(objectStream => {
      ...
      throw new HttpException("Under {" + bucketName + "}, Path {" + prefix + "} doesn't exist!", HttpStatus.BAD_REQUEST)
    }
  }
  ...
  async downloadFileToLocal(bucketName: string, prefix: string, localFilePath: string): Promise<void> {
    const objects = await this.minioService.listObjects(bucketName, prefix);
