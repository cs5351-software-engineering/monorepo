...
export class MinioService {
  ...
  constructor() {
    this.minioClient = new Minio.Client({
      ...
      accessKey: process.env.MINIO_ACCESSLEY,
  ...
  async uploadFile(bucket: string, location: string, file: Express.MulterFile, newfilename?: string) {
    let filename =""
  ...
  async downloadFile(bucket: string, location: string, filename: string) {
    ...
    let filefullpath = UpdateFilePath(path.join(location,filename))
